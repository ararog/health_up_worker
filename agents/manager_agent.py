...
@manager_agent.tool
def list_doctors(ctx: RunContext[ManagerDependencies]) -> list[Doctor]:
    logger.info("Get office doctors...")
    return find_doctors_by_office_id(ctx.deps.office_id)

@manager_agent.tool
def list_specialities(ctx: RunContext[ManagerDependencies]) -> list[Speciality]:
    logger.info("Get office specialists...")
    return find_specilities_by_office_id(ctx.deps.office_id)
  
@manager_agent.tool
def list_appointments(ctx: RunContext[ManagerDependencies]) -> list[Appointment]:
    logger.info("Listing appointments...")
    return list_existing_appointments(ctx.deps.office_id)

@manager_agent.tool
def get_doctor(ctx: RunContext[ManagerDependencies], doctor_name: str) -> Patient:
...
@manager_agent.tool
def get_patient(ctx: RunContext[ManagerDependencies]) -> Patient:
...
@manager_agent.tool
def get_appointment(ctx: RunContext[ManagerDependencies]) -> Appointment:
...
@manager_agent.tool
def create_patient(ctx: RunContext[ManagerDependencies], 
...
@manager_agent.tool
def create_appointment(ctx: RunContext[ManagerDependencies], 
